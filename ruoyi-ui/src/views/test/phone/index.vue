...
<script>
import { listPhone, getPhone, delPhone, addPhone, updatePhone } from "@/api/test/phone";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除手机编号为"' + ids + '"的数据项？').then(function() {
        return delPhone(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('test/phone/export', {
        ...this.queryParams
